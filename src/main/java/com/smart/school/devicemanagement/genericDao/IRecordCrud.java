...
public interface IRecordCrud<T, PK extends Serializable> {
	...
	Criteria createCriteria(final Order order, final Criterion... criterions);

	Criteria createCriteria(final int firstResult, final int maxResults,
			final Order order, final Criterion... criterions);

	// 分页接口
	int getCount(final String hql, final Object... values);

	List<T> getPagedList(final String hql, final int pageSize,
